use crate::utils::nostr_identity::Identity;
...
impl EventUnsigned {
    ...
    /// # Example
    /// ```rust
    /// use nostr_rust::utils::event::EventUnsigned;
    /// use nostr_rust::utils::utils::get_timestamp;
    /// let actual_time = get_timestamp();
    ///
    /// let event = EventUnsigned {
    ...
    /// use std::str::FromStr;
    /// use nostr_rust::utils::event::EventUnsigned;
    /// use nostr_rust::utils::nostr_identity::Identity;
    ///
    /// let mut event = EventUnsigned {
    ///  pub_key: env!("PUBLIC_KEY").to_string(),
    ///  created_at: 0, // Don't use this in production
    ///  kind: 0,
    ///  tags: vec![],
    ...
    /// let event_difficulty = EventUnsigned::count_leading_zero_bits(event_id);
    ...
    /// ```
    pub fn to_event(&mut self, secret_key: &Identity, difficulty_target: u16) -> EventSigned {
...
impl fmt::Display for EventSigned {
